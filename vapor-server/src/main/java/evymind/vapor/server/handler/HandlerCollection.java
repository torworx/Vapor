 ...
 * Derived implementation may alter the order or the conditions of calling the contained handlers.
 * <p>
 * 
	...
	public void handle(Transport transport, Request request, Response response) throws IOException, ServiceException {
		if (_handlers != null && isStarted()) {
			...
			for (int i = 0; i < _handlers.length; i++) {
				try {
					_handlers[i].handle(transport, request, response);
				} catch (IOException e) {
					throw e;
	...
	protected void doStart() throws Exception {
		...
		if (_handlers != null) {
			if (_parallelStart) {
				...
				for (int i = 0; i < _handlers.length; i++) {
					final int h = i;
					getServer().getEventBus().publish(new Runnable() {
						public void run() {
