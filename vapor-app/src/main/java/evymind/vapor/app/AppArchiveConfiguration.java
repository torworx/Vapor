...
public class AppArchiveConfiguration extends AbstractConfiguration {
	 ...
	 * 
	 * @param fileattr
	 *            the file attribute to analyze and return from (supports type File and type String, all others return
	 *            null)
	...
	public void unpack(AppContext context) throws IOException {
		...
		if (appres == null) {
			...
			if (appres.exists()
					&& ((context.isCopyWebDir() && appres.getFile() != null && appres.getFile().isDirectory()) || (context
							.isExtractSAR() && appres.getFile() != null && !appres.getFile().isDirectory()))) {
