...
public final class FS {
	...
	public static void deleteFile(File path) {
		...
		Assert.assertTrue("Can only delete content within the /target/tests/ directory: " + path.getAbsolutePath(),
				FS.isTestingDir(path.getParentFile()));

		Assert.assertTrue("Failed to delete file: " + path.getAbsolutePath(), path.delete());
	}

	private static void recursiveDelete(File dir) {
		Assert.assertTrue("Can only delete content within the /target/tests/ directory: " + dir.getAbsolutePath(),
				FS.isTestingDir(dir));
	...
	protected static boolean isTestingDir(File dir) {
		return dir.getAbsolutePath().startsWith(TestingUtils.getTargetTestingDir().getAbsolutePath());
	}
