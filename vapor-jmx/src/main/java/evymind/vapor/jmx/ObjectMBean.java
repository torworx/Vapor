 ...
 * property bundles discovered by names related to all superclasses and all
 * superinterfaces.
 * 
 * Attributes and methods exported may be "Object" and must exist on the wrapped
 * object, or "MBean" and must exist on a subclass of OBjectMBean or "MObject"
 * which exists on the wrapped object, but whose values are converted to MBean
 * object names.
 * 
 */
...
public class ObjectMBean implements DynamicMBean {
	 ...
	 * <LI>evymind.vapor.core.jmx.ObjectMBean
	 * </UL>
	 * 
	 * @param o
	 *            The object
	...
	public static Object mbeanFor(Object o) {
		try {
			...
			while (mbean == null && oClass != null) {
				...
				try {
					Class mClass = (Object.class.equals(oClass)) ? oClass = ObjectMBean.class : Loader.loadClass(
							oClass, mName, true);
	...
	public MBeanInfo getMBeanInfo() {
		try {
			if (_info == null) {
				...
				for (int i = 0; i < LazyList.size(influences); i++) {
					...
					try {
						...
						while (e.hasMoreElements()) {
							...
							// Determin if key is for mbean , attribute or for
							// operation
							if (key.equals(cName)) {
				...
				_info = new MBeanInfo(o_class.getName(), desc, (MBeanAttributeInfo[]) LazyList.toArray(attributes,
						MBeanAttributeInfo.class), (MBeanConstructorInfo[]) LazyList.toArray(constructors,
						MBeanConstructorInfo.class), (MBeanOperationInfo[]) LazyList.toArray(operations,
						MBeanOperationInfo.class), (MBeanNotificationInfo[]) LazyList.toArray(notifications,
	 ...
	 * @param name
	 * @param metaData
	 *            "description" or "access:description" or
	 *            "type:access:description" where type is one of:
	 *            <ul>
	 *            <li>"Object" The field/method is on the managed object.
	 *            <li>"MBean" The field/method is on the mbean proxy object
	 *            <li>"MObject" The field/method is on the managed object and
	 *            value should be converted to MBean reference
	 *            <li>"MMBean" The field/method is on the mbean proxy object and
	 *            value should be converted to MBean reference
	 *            </ul>
	...
	public MBeanAttributeInfo defineAttribute(String name, String metaData) {
		...
		try {
			...
			if (convert) {
				_convert.add(name);
				if (type.isArray())
					info = new MBeanAttributeInfo(name, OBJECT_NAME_ARRAY_CLASS, description, getter != null,
							setter != null, getter != null && getter.getName().startsWith("is"));

				else
					info = new MBeanAttributeInfo(name, OBJECT_NAME_CLASS, description, getter != null, setter != null,
							getter != null && getter.getName().startsWith("is"));
			} else
	 ...
	 * "name(signature)[n]", the returned description is for the last parameter
	 * of the partial signature and is assumed to start with the parameter name,
	 * followed by a colon.
	 * 
	 * @param metaData
	 *            "description" or "impact:description" or
	 *            "type:impact:description", type is the "Object","MBean",
	 *            "MMBean" or "MObject" to indicate the method is on the object,
	 *            the MBean or on the object but converted to an MBean
	 *            reference, and impact is either "ACTION","INFO","ACTION_INFO"
	 *            or "UNKNOWN".
	 */
	private MBeanOperationInfo defineOperation(String signature, String metaData, ResourceBundle bundle) {
		...
		try {
			...
			for (i = 0; i < args.length; i++) {
				Class type = Types.fromTypeName(args[i]);
				if (type == null)
					type = Thread.currentThread().getContextClassLoader().loadClass(args[i]);
				types[i] = type;
				args[i] = type.isPrimitive() ? Types.toTypeName(type) : args[i];
			...
			return new MBeanOperationInfo(method_name, description, pInfo, returnClass.isPrimitive() ? Types.toTypeName(returnClass) : (returnClass.getName()), impact);
		} catch (Exception e) {
